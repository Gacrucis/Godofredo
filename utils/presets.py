...
class TimeLine(VGroup):
    def __init__(
        ...
        time_buff: float = None,
        time_scale: float = None,
        dot_scale: float = None,
        dot_colors: list = None,
        arrow_buff: float = None,
        ...
        **kwargs,
        ...
        self.index_reference = {"times": 0, "dots": 0}
        ...
        self.line = Line(
            start=self.initial_point, end=self.end_point, z_index=0, **line_config
        )
        ...
        self.arrow = Arrow(
            start=ORIGIN, end=self.directions["arrow_direction"], **arrow_config
        ).scale(arrow_scale)
        ...
        self.add(self.line, *self.dots, *self.times, self.arrow)
    ...
    def position_dots(self) -> None:
        ...
        for n, dot in enumerate(dots):
            dot.move_to(line.point_from_proportion(point_distance * n))
    ...
    def position_times(self) -> None:
        ...
        for n, time in enumerate(times):
            time.move_to(line.point_from_proportion(point_distance * n))
    ...
    def create(
        self, with_arrow: bool = False, with_time: bool = False
    ) -> AnimationGroup:
        animations = [DrawBorderThenFill(self.line), DrawBorderThenFill(self.dots)]
        if with_arrow:
            animations.append(DrawBorderThenFill(self.arrow))

        if with_time:
            self.index_reference["times"] += 1  # increased next time index
            animations.append(Write(self.times[0]))
    ...
    def next_time(self) -> AnimationGroup:
        ...
        animations = [
            self.arrow.animate.next_to(
                next_dot, self.directions["arrow"], buff=self.arrow_buff
            ),
            Write(target_time),
        ]
        ...
        if current_time is first_time:
            ...
            current_time.add_updater(
                lambda mob: mob.next_to(
                    first_dot, self.directions["times"], self.time_buff
                )
            )
    ...
    def preload_for_scene(
        self, target_time: str, scene: "Scene", with_arrow: bool = True
    ) -> None:
        """
        ...
        [self._attach_to_dot(time, dot) for time, dot in zip(previous_times, dots)]

        scene.add(line, dots, *previous_times)
...
class PTex(Tex):
    def __init__(
        ...
        **kwargs,
        ...
        """
        line_separated = [
            line + r"\\" for line in self.text_to_lines(text, line_length=line_length)
        ]
        super().__init__(*line_separated, tex_environment=tex_environment, **kwargs)
    ...
    def text_to_lines(self, text: str, line_length: int = 40) -> list:
        ...
        if current_word_index <= (len(text_words) - 1):
            text_lines.append(text_words[current_word_index:])
...
def create_timeline(
    dot_amount=5,
    dot_scale=1,
    lenght=Camera(None).frame_width - 4,
    height_buff=1.3,
    dot_colors=None,
    ...
    timeline_line = Line(
        start=initial_point, end=initial_point.get_center() + RIGHT * lenght
    )
...
def create_bullet_point(
    text, text_color=WHITE, bullet_scale=0.8, bullet_color=None, bullet_buff=0.2
):
    ...
    text_mobject = Tex(text, color=text_color)
...
def create_chapter(
    title, subtitle, color=MAROON_C, subtitle_buff=-0.5, scale_factor=0.7
):
    ...
    # anims.append(FadeOut(title, shift=DOWN))
    anims.append(Wait())
