...
class England(MovingCameraScene):
    def __init__(self, *args, **kwargs):
        ...
        self.timeline.preload_for_scene(
            target_time="Imperio Romano", scene=self  # pass the scene as parameter
        )
        ...
        self.previous = {
            "image": ImageMobject(
                filename_or_array=presets.image_path(".\\history\\13_romans_2.jpg")
            ),
            "title": Tex("Imperio romano", **title_config),
            "list": BulletedList(
        ...
        self.previous["image"].scale(0.9).next_to(self.previous["list"], LEFT, buff=0.4)
    ...
    def construct(self):
        ...
        self.play(
            ...
            FadeOutAndShift(
                VGroup(self.previous["title"], self.previous["list"]), UP_SCREEN
            ),
            run_time=3,
        )
...
class XVICentury(GraphScene):
    def construct(self):
        ...
        timeline.preload_for_scene(
            target_time="1066", scene=self  # pass the scene as parameter
        )
        ...
        self.graph_origin = (
            paragraph.get_corner(RIGHT) + RIGHT * frame_width * 0.5
        ) / 2
        self.x_axis_width = 5
        self.x_min = -2.5
        self.x_max = 2.5
...
class Colombia1500(GraphScene):
    def construct(self):
        ...
        timeline.next_to(REFERENCE_POINT, DOWN, buff=0)
        timeline.preload_for_scene(
            target_time="Siglo XVI", scene=self  # pass the scene as parameter
        )
...
class Colombia1800(GraphScene):
    def construct(self):
        ...
        remaining_space = (Point().align_on_border(RIGHT, buff=0).get_center()) - (
            bullet_points.get_corner(RIGHT)
        )
...
class XXCentury(GraphScene):
    def construct(self):
        ...
        timeline.next_to(REFERENCE_POINT, DOWN, buff=0)
        timeline.preload_for_scene(
            target_time="1800", scene=self  # pass the scene as parameter
        )
        ...
        images = ["laplace.jpg", "fisher.jpg", "pearson.jpg", "stats_modern.svg"]
...
class Colombia1900(GraphScene):
    def construct(self):
        ...
        timeline.next_to(REFERENCE_POINT, DOWN, buff=0)
        timeline.preload_for_scene(
            target_time="Siglo XX", scene=self  # pass the scene as parameter
        )
...
class Colombia2020(GraphScene):
    def construct(self):
        ...
        timeline.next_to(REFERENCE_POINT, DOWN, buff=0)
        timeline.preload_for_scene(
            target_time="2000", scene=self  # pass the scene as parameter
        )
