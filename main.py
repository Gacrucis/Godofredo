...
class Outro(Scene):
    ...
    def construct(self):
        ...
        student_info = VGroup(
            MathTex(r"\gamma \text{ Edward Parada }", color=PURPLE,  **self.names_config),
            MathTex(r"\Omega \text{ Gian Estevez }", color=LIGHT_PURPLE, **self.names_config),
            MathTex(r"\varepsilon \text{ José Silva }", color=VIOLET, **self.names_config),
            MathTex(r"\mu \text{ Yuri Garcia }", color=DARK_SKY_BLUE, ** self.names_config),
        ).scale(author_scale)       
        ...
        # yuri.scale(author_scale).next_to(jose, RIGHT, buff=student_buff)
        # ed.scale(author_scale).next_to(yuri, RIGHT, buff=student_buff)
        # gian.scale(author_scale).next_to(ed, RIGHT, buff=student_buff)

        # student_info[0].next_to(ed, UP, buff=.3)
        # student_info[1].next_to(gian, UP, buff=.3)
        # student_info[2].next_to(jose, UP, buff=.3)
        # student_info[3].next_to(yuri, UP, buff=.3)
